...
// =====================================================================
// STEP 1: Gemini analyzes photo → returns shape regions
// =====================================================================
async function analyzeImage(b64, mime) {
  try {
    const prompt = `Analyze this design image for embroidery digitization.
...
Return ONLY JSON with this exact structure:
{
  ...
  "shapes": [
    {
      "label": "description of this part",
      "color": "#RRGGBB",
      "x": 0,
      "y": 0,
      "w": 100,
...
2. Break the design into logical COLOR REGIONS (background, main element, text area, border, accent)
3. Each shape gets a solid dominant color — ignore gradients and shadows
4. Position x,y,w,h use 0-100 scale (percentage of image)
5. stitch_type: "fill" for large areas, "satin" for borders/edges, "running" for thin lines/text
6. Aim for 2-6 shapes total
7. Colors must be actual hex codes from the image

Example for a cigarette pack with red body, gold stripe, white text:
{
  "shapes": [
    {"label": "Red pack body", "color": "#C41E3A", "x": 5, "y": 10, "w": 90, "h": 75, "stitch_type": "fill"},
    {"label": "Gold stripe", "color": "#FFD700", "x": 5, "y": 50, "w": 90, "h": 8, "stitch_type": "satin"},
    {"label": "White text area", "color": "#FFFFFF", "x": 15, "y": 15, "w": 70, "h": 25, "stitch_type": "running"}
  ]
}`;
    ...
    return {
      ...
      description: "Design",
      shapes: [
        { label: "Main area", color: "#C41E3A", x: 5, y: 5, w: 90, h: 90, stitch_type: "fill" }
      ]
    };
  }
}
...
// =====================================================================
// STEP 2: Convert shapes → real stitch coordinates
// =====================================================================
function generateStitches(analysis) {
  ...
  for (const shape of shapes) {
    ...
    if (shape.stitch_type === "satin") {
      // Zigzag border
      const count = Math.floor((sw + sh) * 2 / 2);
      for (let i = 0; i < count; i++) {
        ...
        else if (side === 1) { ox = sx + sw; oy = sy + sh * st; }
        else if (side === 2) { ox = sx + sw * (1 - st); oy = sy + sh; }
        else { ox = sx; oy = sy + sh * (1 - st); }
      ...
      // Single outline line
      const count = Math.floor((sw + sh) * 2 / 3);
      for (let i = 0; i < count; i++) {
        const t = (i / count) * 4;
        const side = Math.floor(t) % 4;
        const st = t - Math.floor(t);
        ...
        else if (side === 1) { px = sx + sw; py = sy + sh * st; }
        else if (side === 2) { px = sx + sw * (1 - st); py = sy + sh; }
        else { px = sx; py = sy + sh * (1 - st); }
        stitches.push({ x: px, y: py, color, type: i === 0 ? "jump" : "stitch" });
      }
    } else {
      // Fill: horizontal scanlines
      const spacing = 2;
      const rows = Math.max(3, Math.floor(sh / spacing));
      for (let r = 0; r < rows; r++) {
        const rowY = sy + (r / rows) * sh;
        // Find intersections with shape edges
        const ints = [];
        const pts = [
          { x: sx, y: sy }, { x: sx + sw, y: sy },
          { x: sx + sw, y: sy + sh }, { x: sx, y: sy + sh }, { x: sx, y: sy }
        ];
        for (let i = 0; i < 4; i++) {
