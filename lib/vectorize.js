...
function absorbSlivers(pm,W,H,darkCi,magCi,opts={}){
  ...
  for(let pass=0;pass<4;pass++){  // 4 passes: catches fragments that emerge after first-pass merges create new small components
    ...
    for(let c=1;c<=nc;c++){
      ...
      else absorb = (a<_minA) || (a<_maxA && a<per*_perR);  // tighter: absorbs larger fragments + thin strands (high perimeter-to-area)
      if(!absorb)continue;
...
function bridgeSameColorGaps(pm,W,H,nColors,maxGap=6){
  ...
  for(const {c} of order){
    ...
    for(let i=0;i<W*H;i++){
      if(dist[i]>0&&dist[i]<=maxGap&&pm[i]<0&&!claimed[i]){
        out[i]=c;
        claimed[i]=1;
      }
    }
...
async function vectorizeToDST(cleanedBuffer, colors, canvasSize, pxPerMm, params={}) {
  ...
  bridgeSameColorGaps(pixMap,W,H,colors.length,tune.bridgeMaxGap);  // merge nearby same-color regions to reduce fragmentation
    ...
    if(n>0){
      ...
      if(Math.max(bw/W,bh/H)<0.8){
        ...
        try{
          ...
          bridgeSameColorGaps(pm2,W,H,colors.length,tune.bridgeMaxGap);  // merge same-color gaps after auto-zoom re-quantization
          pixMap.set(pm2);
