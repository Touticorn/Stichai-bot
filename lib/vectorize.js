...
function hardCap(stitches, maxStitch, trimGap, minStitch) {
  const TRAVEL_CUT = Math.min(trimGap, 120);   // bridge fill-internal gaps ≤12mm (was 8, then 10).
...
async function vectorizeToDST(cleanedBuffer, colors, canvasSize, pxPerMm, params={}) {
  ...
  const trimGap  = 9*pxPerMm;
  const travelCut= 12*pxPerMm;   // bridge gaps ≤12mm (was 10mm, originally 8mm) - target trims<2%
