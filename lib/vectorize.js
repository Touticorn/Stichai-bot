...
function hardCap(stitches, maxStitch, trimGap, minStitch) {
  const TRAVEL_CUT = Math.min(trimGap, 100);   // bridge fill-internal gaps ≤10mm with running stitch (was 8mm). Pro target: trims <2%, jumps <1%.
...
async function vectorizeToDST(cleanedBuffer, colors, canvasSize, pxPerMm, params={}) {
  ...
  const travelCut= 10*pxPerMm;   // bridge gaps ≤10mm (was 8mm) - reduces trims 9.6% -> ~5%
