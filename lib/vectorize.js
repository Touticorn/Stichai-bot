...
async function buildMaskPNG(sharp, pixMap, ci, W, H, tuckZone) {
  ...
  if(tuckZone){
    for(let it=0;it<6;it++){   // more tuck into the dark band: seats fills under outlines + closes outlined seams (neck/jaw, between figures) WITHOUT growing into open background
      const prev=c2.slice();
...
function hardCap(stitches, maxStitch, trimGap, minStitch) {
  const TRAVEL_CUT = Math.min(trimGap, 80);   // aligned with 8mm travelCut
  ...
  for(const s of stitches){
    if(prev && s.type!=="trim"){
      ...
      } else if(d<minStitch && s.type==="fill"){ continue; }
    }
    out.push(s); prev=s;
  }
  return out;
...
async function vectorizeToDST(cleanedBuffer, colors, canvasSize, pxPerMm, params={}) {
  ...
  const travelCut= 8*pxPerMm;   // bridge gaps ≤8mm with travel stitches (was 4mm → too many trims)   
  ...
  for(let ci=0;ci<colors.length;ci++){
    ...
    const _td = (ci===darkCi) ? Math.round(MIN_AREA*(P.darkFeatureFrac??1.0)) : Math.round(MIN_AREA);
    ...
    const _minSub = (ci===darkCi) ? MIN_SUBPATH*(P.darkFeatureFrac??1.0) : MIN_SUBPATH;
  ...
  function addLocks(sts,minStitch){
    ...
    const lock=(x,y,color)=>{out.push({x:x+lockStep,y:y,color,type:"running"},{x:x-lockStep,y:y,color,type:"running"},{x:x,y:y,color,type:"running"});};
  ...
  function coalesceShortSegments(sts,minStitch){
    if(!sts.length) return sts;
    const out=[sts[0]];
    for(let i=1;i<sts.length;i++){
      const s=sts[i], prev=out[out.length-1];
      const d=Math.hypot(s.x-prev.x,s.y-prev.y);
      if(d<minStitch && s.type!=="trim" && prev.type!=="trim" && s.color===prev.color && s.type!=="running" && prev.type!=="running"){
        prev.x=Math.round((prev.x+s.x)/2); prev.y=Math.round((prev.y+s.y)/2);
        if(s.type==="fill" || prev.type==="fill") prev.type="fill";
      }else{ out.push(s); }
    }
    return out;
  }
  const capped=coalesceShortSegments(addLocks(hardCap(allStitches,maxStitch,trimGap,minStitch),minStitch),minStitch);
