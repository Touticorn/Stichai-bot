...
async function buildMaskPNG(sharp, pixMap, ci, W, H, tuckZone) {
  ...
  const c2=dilateBin(m, tuckZone ? 1 : 3);   // dark outline grows +3px to cover grown/tucked fills (contour registration); fills stay +1
...
function hardCap(stitches, maxStitch, trimGap) {
  const TRAVEL_CUT = Math.min(trimGap, 40);   
