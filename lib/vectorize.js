...
function absorbSlivers(pm,W,H,darkCi,magCi){
  for(let pass=0;pass<4;pass++){  // 4 passes: catches fragments that emerge after first-pass merges create new small components
    ...
    for(let c=1;c<=nc;c++){
      ...
      let absorb;
      if(col===magCi) absorb = a<3000;
      else absorb = (a<250) || (a<20000 && a<per*4.0);  // tighter: absorbs larger fragments + thin strands (high perimeter-to-area)
      if(!absorb)continue;
...
async function vectorizeToDST(cleanedBuffer, colors, canvasSize, pxPerMm, params={}) {
  ...
  bridgeSameColorGaps(pixMap,W,H,colors.length,8);  // merge nearby same-color regions to reduce fragmentation

  {
    ...
    if(n>0){
      ...
      if(Math.max(bw/W,bh/H)<0.8){
        const mar=0.01;  // reduced from 0.04 to prevent edge artifacts (vertical bar)
        ...
        try{
          ...
          bridgeSameColorGaps(pm2,W,H,colors.length,8);  // merge same-color gaps after auto-zoom re-quantization
          pixMap.set(pm2);
  ...
  for(let ci=0;ci<colors.length;ci++){
    ...
    const _td = (ci===darkCi) ? Math.round(MIN_AREA*(P.darkFeatureFrac ?? (process.env.STICHAI_DARK_FEAT_FRAC ? parseFloat(process.env.STICHAI_DARK_FEAT_FRAC) : 0.6))) : Math.round(MIN_AREA);
