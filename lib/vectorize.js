...
async function vectorizeToDST(cleanedBuffer, colors, canvasSize, pxPerMm, params={}) {
  ...
  const tune = {
    bridgeMaxGap:      T.bridgeMaxGap      ?? parseInt(process.env.STICHAI_BRIDGE_GAP,10)     ?? 8,
    absorbMinArea:     T.absorbMinArea     ?? parseInt(process.env.STICHAI_ABSORB_MIN,10)     ?? 250,
    absorbMaxArea:     T.absorbMaxArea     ?? parseInt(process.env.STICHAI_ABSORB_MAX,10)     ?? 20000,
    absorbPerimRatio:  T.absorbPerimRatio  ?? parseFloat(process.env.STICHAI_ABSORB_RATIO)    ?? 4.0,
    autoZoomMargin:    T.autoZoomMargin    ?? parseFloat(process.env.STICHAI_ZOOM_MARGIN)     ?? 0.01,
    darkUnifyLum:      T.darkUnifyLum      ?? parseInt(process.env.STICHAI_DARK_LUM,10)        ?? 58,
    darkUnifyThresh:   T.darkUnifyThresh   ?? parseInt(process.env.STICHAI_DARK_THRESH,10)    ?? 95,
    potraceTurdSize:   T.potraceTurdSize   ?? parseInt(process.env.STICHAI_POTRACE_TURD,10)   ?? 10,
  };
