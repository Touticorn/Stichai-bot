...
function traceMask(pngBuf, opts={}) {
  return new Promise((res,rej) => getPotrace().trace(pngBuf, {
    turdsize: opts.turdsize ?? 8, threshold: 128,
    turnpolicy: "black", optcurve: true, alphamax: 1.0, opttolerance: 0.2,
    ...opts
  }, (e,s)=>e?rej(e):res(s)));
}
...
function parseSVGPathGroups(svg) {
  ...
  while ((m=re.exec(svg))!==null) {
    ...
    if (polys.length) groups.push(polys);   // one group per <path d="...">
  }
  return groups;
}
