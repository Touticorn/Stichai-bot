...
async function vectorizeToDST(cleanedBuffer, colors, canvasSize, pxPerMm, params={}) {
  ...
  const pullComp = Math.max(1,Math.round(0.1*pxPerMm)); // 0.1mm pull comp
  ...
  for(let ci=0;ci<colors.length;ci++){
    ...
    const areaPx=filtered.reduce((s,g)=>s+g[0].length,0);
    colorRegions.push({ci,color:colors[ci],groups:filtered,areaPx});
  }
