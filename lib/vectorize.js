...
async function vectorizeToDST(cleanedBuffer, colors, canvasSize, pxPerMm, params={}) {
  ...
  for(let ci=0;ci<colors.length;ci++){
    ...
    const minUnitArea = Math.max(100, Math.round(W * H * 0.0001));
    const bigUnits = units.filter(g => g[0].length >= minUnitArea);
    if (bigUnits.length < units.length) {
      console.log(`[vec] ci=${ci} dropped ${units.length - bigUnits.length} tiny units (kept ${bigUnits.length})`);
    }
    ...
    // eyebrows are intentional but small).
    const speckThreshold = Math.max(200, Math.round(W * H * 0.0004));
    if (ci !== darkCi && areaPx < speckThreshold) {
      console.log(`[vec] ci=${ci} dropped as speck (areaPx=${areaPx} < ${speckThreshold})`);
      continue;
    }
