...
function removeBorderBackground(pm,W,H){
  ...
  for(const s0 of seeds){
    ...
    if(px.length > W*H*0.02){ for(const i of px) pm[i]=-1; }
  }
}
...
function absorbSlivers(pm,W,H,darkCi,magCi){
  for(let pass=0;pass<2;pass++){
    ...
    for(let s=0;s<W*H;s++){
      ...
      let bi=-1,bc=0;
      for(let k=0;k<18;k++){if(cnt[k]>bc){bc=cnt[k];bi=k-1;}}
    ...
    for(let c=1;c<=nc;c++){
      ...
      if(bn<0||col===darkCi)continue;
      let absorb;
      if(col===magCi) absorb = a<2000;
      else absorb = (a<150) || (a<5000 && a<per*3.0);
      if(absorb){tgt[c]=bn;changed++;}
    }
    if(!changed)break;
    for(let i=0;i<W*H;i++){const c=lbl[i];if(tgt[c]>=0)pm[i]=tgt[c];}
  }
}
...
async function vectorizeToDST(cleanedBuffer, colors, canvasSize, pxPerMm, params={}) {
  ...
  removeBorderBackground(pixMap,W,H);
