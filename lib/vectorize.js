...
async function vectorizeToDST(cleanedBuffer, colors, canvasSize, pxPerMm, params={}) {
  ...
  for(let ci=0;ci<colors.length;ci++){
    ...
    const _td = (ci===darkCi) ? Math.round(MIN_AREA*(P.darkFeatureFrac??0.3)) : Math.round(MIN_AREA);
    ...
    const _minSub = (ci===darkCi) ? MIN_SUBPATH*(P.darkFeatureFrac??0.3) : MIN_SUBPATH;
