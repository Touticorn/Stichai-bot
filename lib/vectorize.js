...
async function buildMaskPNG(sharp, pixMap, ci, W, H, tuckZone) {
  ...
  const c2=dilateBin(m, tuckZone ? 2 : 2);   // fills +2 (was +1): extend fills to meet their outlines/neighbours so the dark outline sits on the edge and seam gaps (e.g. neck/jaw, between figures) close; dark grows +2 to cover.

  if(tuckZone){
    for(let it=0;it<4;it++){   // was 3: a touch more tuck into the dark band to seat fills under outlines
      const prev=c2.slice();
