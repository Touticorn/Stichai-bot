  ...
  for(const{ci,color,groups}of colorRegions){
    ...
    if(isDark){
      ...
      for(let pass=0;pass<groups.length;pass++){
        ...
        // Perimeter satin pass: re-trace silhouette at very dense step (0.2mm)
        // to smooth stair-step artifacts from clipped fill rows crossing curved
        // face contours. Cost ≈ 30-60m of needle travel per region.
        if (USE_PERIM_PASS && aMm2 >= 25) {
          const perim = boundaryWalk([outer], color, Math.max(3, Math.round(0.2 * pxPerMm)), trimGap, 0);
