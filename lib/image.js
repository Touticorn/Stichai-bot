...
async function extractColorsFromUnmasked(imageBuffer, maskBuffer, canvasSize, maxColors) {
  ...
  // LUMINANCE_GUARD: also keep buckets separate if their luminance differs by
  // >= 25 points. This prevents gray hair (lum~88) from merging into dark
  // outline (lum~32) when their Lab distance is small but they represent
  // visually distinct regions (hair vs outline).
  const MIN_AREA_PCT = 0.04;
  const LUM_GAP = 25;
  const lum = (lab) => Math.round(lab.l);
  const selected = [];
  for (const bucket of allBuckets) {
    if (selected.length >= maxColors) break;
    const tooSimilar = selected.find(s => {
      const dist = dE(bucket.lab, s.lab);
      if (dist >= MIN_DIST) return false;
      // Within Lab distance — check luminance gap
      const dl = Math.abs(lum(bucket.lab) - lum(s.lab));
      if (dl >= LUM_GAP) return false;  // different luminance = keep separate
      return true;
    });
    if (tooSimilar) {
