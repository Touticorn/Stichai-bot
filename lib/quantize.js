...
async function quantizeBuffer(imageBuffer, nColors = 8) {
  ...
  try {
    ...
    for (let i = 0; i < sampLen; i++) {
      const r = sampleRaw[i*3], g = sampleRaw[i*3+1], b = sampleRaw[i*3+2];
      // Chroma-key magenta and pure white (cartoon letterbox / background).
      // Without this filter the sample is dominated by magenta to the extent
      // every median-cut bin collapses to a magenta centroid, so the
      // returned centroids are 6-8 copies of the SAME hex.
      const isMagenta = r > 150 && b > 150 && g < 90 && Math.abs(r - b) < 60;
      const isPureWhite = r >= 248 && g >= 248 && b >= 248;
      const isPureBlack = r <= 8 && g <= 8 && b <= 8;
      if (isMagenta || isPureWhite || isPureBlack) continue;
      sampleFiltered.push(r, g, b, 255);
    }
    ...
    for (let i = 0; i < oTotal; i += oChan) {
      ...
      // Preserve magenta backdrop: do NOT snap it to a centroid, so the downstream
      // chroma-keys (buildPixelMap / dropBackgroundRegions) can drop it as no-stitch.
    ...
    return { buffer: imageBuffer, centroids: [] };
  }
}

/**
 * Return ONLY centroids (RGB hex strings) for a buffer without re-encoding.
 * Useful when caller just wants the palette without rewriting pixels.
 */
async function quantizeBufferCentroids(imageBuffer, nColors = 8) {
  if (!imageBuffer || !nColors || nColors < 2 || nColors > 30) return [];
  try {
    const sampleMax = 320;
    const meta = await sharp(imageBuffer).metadata();
    const ratio = Math.min(1.0, sampleMax / Math.max(meta.width || sampleMax, meta.height || sampleMax));
    ...
    if (sampleFiltered.length < 16) return [];
    const centroids = medianCut(Uint8Array.from(sampleFiltered), nColors);
...
module.exports = { quantizeBuffer, quantizeBufferCentroids };
