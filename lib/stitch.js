...
function v70_buildShapes(pixMap, colors, canvasSize, pxPerMm) {
  ...
  // Defragment fills cut apart by dark outlines BEFORE finding regions. This is
  // the root-cause fix for over-fragmentation (60-120 regions → ~10-20).
  v70_closeColorMasks(pixMap, colors, canvasSize);
  ...
  /* ── NUCLEAR: merge all same-color regions into ONE shape per color ──────
     Commercial digitizers treat "all the cream" as ONE fill, regardless of
     internal outlines cutting through it. Connected-component finding produces
     40-100 fragments because every dark outline severs the colour. By merging
     all regions of the same palette index into one super-region, we get ~7
     shapes (one per colour) instead of 57-116 fragments.

     The fill scanner (v70_scanRuns) only stitches where mask=1, so it
     naturally handles the merged mask's gaps (outline pixels) — each row's
     fill runs cover the actual garment shape. The definition outline traces
     the outer boundary of each merged shape, giving clean contours.

     This eliminates fragmentation, slashes jumps (one travel per colour, not
     per fragment), and ensures every colour area fills solidly. */
  const byCi = new Map();
  for (const reg of rawRegions) {
    if (!byCi.has(reg.ci)) byCi.set(reg.ci, []);
    byCi.get(reg.ci).push(reg);
  }
  const mergedRegions = [];
  for (const [ci, regs] of byCi) {
    const allPts = [];
    for (const r of regs) {
      for (const p of r.pts) allPts.push(p);
    }
    if (allPts.length >= minAreaPx) {
      mergedRegions.push({ ci, pts: allPts });
    }
  }
  console.log(`[v70] Merged to ${mergedRegions.length} colour groups (from ${rawRegions.length} connected regions)`);
  ...
  // Log each shape's details to diagnose empty fills
