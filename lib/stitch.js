...
function v70_buildShapes(pixMap, colors, canvasSize, pxPerMm, params) {
  const P = params || {};
  const tune = P.tune || {};
  const minAreaPx = Math.max(200, Math.round((tune.v70MinArea || 2.0) * pxPerMm * pxPerMm));
...
function v72_buildAndGenerate(pixMap, colors, canvasSize, pxPerMm, params) {
  ...
  const pxScale = canvasSize / 800;
  const tune = P.tune || {};
  const minAreaPx = Math.max(200, Math.round((tune.v72MinArea || 2.0) * pxPerMm * pxPerMm));
  ...
  const _minFragPx = Math.max(minAreaPx, Math.round((P.tune && P.tune.v72FragPx || 1.5) * pxPerMm * pxPerMm)); // step5: drop stray fragments
