    ...
    for (const sh of group) {
      ...
      if (strat === "running") {
        ...
        for (let y = 0; y < sh.h; y++) for (let x = 0; x < sh.w; x++) {
          ...
          const u = ((x + sh.offX) - sh.pca.cx) * lc + ((y + sh.offY) - sh.pca.cy) * ls;
          if (u < uMin) uMin = u;
          if (u > uMax) uMax = u;
        }
        const A = { x: sh.pca.cx + lc * uMin, y: sh.pca.cy + ls * uMin };
        const B = { x: sh.pca.cx + lc * uMax, y: sh.pca.cy + ls * uMax };
  ...
  if (shapes.length >= 2) {
    ...
    for (let i = 0; i < shapes.length && cCount < maxC; i++) {
      ...
      for (let j = i + 1; j < shapes.length && cCount < maxC; j++) {
        ...
        const cAx = A.pca.cx, cAy = A.pca.cy;
        const cBx = B.pca.cx, cBy = B.pca.cy;
        const segs = 8;
