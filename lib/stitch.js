...
function v72_buildAndGenerate(pixMap, colors, canvasSize, pxPerMm, params) {
  ...
  const { fillMap, outlineMask, outlineCi } = v72_removeOutlines(pixMap, colors, canvasSize);
  const regions = v70_findRegions(fillMap, canvasSize, canvasSize, minAreaPx);
  console.log(`[v72] Regions after outline removal: ${regions.length}`);

  const shapes = [];
  for (const reg of regions) {
    if (reg.ci === outlineCi) continue;
    const pts = reg.pts;
    if (pts.length < minAreaPx) continue;
    const pca = v70_pca(pts);
    const cls = v70_classify(pts, pca, pxPerMm);
  ...
  // Order colours by total area DESC (largest base layers sew first → back-to-front)
  const areaByCi = {};
  ...
  const pRow      = Math.max(3, Math.round(0.5 * 10));
  const pPullComp = Math.round((P.pullComp || 2) * pxScale);
  const pBrick    = Math.round(pRow * 0.4);
  const pMaxBridge= Math.round(250 * pxScale);
  const maxStitch = Math.max(20, Math.round(35 * pxScale * 0.75));
  const ulSpacing = Math.round(pRow * 3);

  const out = [];
  const colorCounts = colors.map(() => ({ fill: 0, satin: 0, running: 0, underlay: 0 }));

  for (const ci of ciOrder) {
    const group = v72_nnOrder(shapes.filter(s => s.ci === ci));
    for (const sh of group) {
      const color = sh.color;
      const angle = (sh.pca.aspect < 2.2) ? Math.PI / 2 : sh.pca.angle;

      if (sh.type === "fill" || (sh.type === "satin" && sh.widthMm > 3.5)) {
        // UNDERLAY first (foundation — the thing auto-digitizers skip)
        if (sh.areaMm2 > 8) {
          const ul = generateZigzagUnderlay(fillMap, sh.reg, ci, canvasSize, color, ulSpacing, maxStitch);
        ...
        if (fs.length) {
          ...
          for (const t of generateTieStitches(last.x, last.y, color, -1, 0)) out.push(t);
        }
      } else {
        const path = v70_traceOutline(sh.mask, sh.w, sh.h);
        if (path && path.length) {
  ...
  // OUTLINE LAST — the dark contour on top, like a human digitizer
  if (outlineCi >= 0) {
    const olStitches = v72_outlineMaskToRunning(outlineMask, canvasSize, colors[outlineCi], maxStitch, minAreaPx);
    for (const s of olStitches) { out.push(s); if (s.type !== "trim") colorCounts[outlineCi].running++; }
    console.log(`[v72] outline: ${olStitches.filter(s=>s.type!=='trim').length} running stitches in ${colors[outlineCi]}`);
  }
