  ...
  for (let ci = 0; ci < colors.length; ci++) {
    ...
    for (const sh of group) {
      ...
      if (strat === "running") {
        ...
        const localPRow = (() => {
          const a = sh.areaMm2;
          if (a <  3)  return Math.max(2, Math.round(1.0 * pxScale));
          if (a < 15)  return Math.max(2, Math.round(0.7 * pxScale));
          if (a < 40)  return Math.max(3, Math.round(0.5 * pxScale));
          if (a < 90)  return Math.max(3, Math.round(0.6 * pxScale));
          return         Math.max(4, Math.round(0.75 * pxScale));
        })();
