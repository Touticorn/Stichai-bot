...
/* ── Tier-2: contour-aligned fill direction ─────────────────────────────
 *
 * Many cartoon regions are roughly equidistant (aspect < 2.2): faces,
 * cheeks, garment folds. PCA returns "no clear axis" and the code
 * defaults to Math.PI/2 (pure vertical) — so tatami rows stroke
 * uniformly top-to-bottom across the WHOLE figure, ignoring the
 * shape's own curvature.
 *
 * Better fallback: trace the boundary, collect the dominant straight
 * edge orientation, and stitch across the SHORT direction of that
 * dominant edge. Shoulders curve into arms -> stitches fan with the
 * contour. Cheek bubble -> stitches radiate from the bulge.
 *
 * Returns angle in radians suitable for v70_scanRuns().
 */
function v70_contourAlignedAngle(mask, w, h, fallbackAngle) {
  const path = v70_traceOutline(mask, w, h);
  if (path.length < 8) return fallbackAngle;
  // Sample boundary edges using stride S. Edge vector = path[i+S] - path[i].
  const S = Math.max(2, Math.round(path.length / 60));
  const NBINS = 24;            // 360/24 = 15° bins
  const weights = new Float64Array(NBINS);
  const counts  = new Float64Array(NBINS);
  let straightSum = 0;
  for (let i = 0; i + S < path.length; i++) {
    const [x0, y0] = path[i];
    const [x1, y1] = path[i + S];
    const dx = x1 - x0, dy = y1 - y0;
    const len = Math.hypot(dx, dy);
    if (len < 1) continue;
    // Edge direction is (dx,dy)/len. We want stitch rows perpendicular to
    // the LONGEST straight edge. The angle of the edge vector goes in
    // a bin; weight = edge length inversely weighted by neighbour-edge
    // straightness so curls don't win the vote.
    const a = Math.atan2(dy, dx);
    let bin = Math.round((a / Math.PI) * (NBINS / 2)) % NBINS;
    if (bin < 0) bin += NBINS;
    const prev = i - S < 0 ? path[path.length + (i - S)] : path[i - S];
    const [px0, py0] = prev;
    const pdx = x0 - px0, pdy = y0 - py0;
    const prevLen = Math.hypot(pdx, pdy) || 1;
    const cos = (dx * pdx + dy * pdy) / (len * prevLen || 1);
    const straight = (cos + 1) / 2;  // 0..1
    const w = len * (0.3 + 0.7 * straight);
    weights[bin] += w;
    counts[bin]  += 1;
    straightSum += straight;
  }
  // Find the top-2 dominant bins; if they agree within 2 bins use the
  // average, otherwise pick the heavier one (longer straight edge).
  let bestIdx = 0, bestVal = 0;
  for (let i = 0; i < NBINS; i++) if (weights[i] > bestVal) { bestVal = weights[i]; bestIdx = i; }
  // merged-bin: include neighbour-bin which kept its vote
  const totalBins = (e) => e.reduce((s, v) => s + v, 0);
  const wTotal = totalBins(weights);
  if (wTotal < 1) return fallbackAngle;
  // Take top two non-adjacent peaks within distance 4 bins
  const peaks = [];
  for (let i = 0; i < NBINS; i++) if (weights[i] > wTotal * 0.10) peaks.push({ i, w: weights[i] });
  let chosenI = bestIdx;
  if (peaks.length >= 2) {
    peaks.sort((a, b) => b.w - a.w);
    const a = peaks[0].i, b = peaks[1].i;
    if (Math.abs(a - b) <= 4) {
      // Average direction with weight
      const aw = weights[a], bw = weights[b];
      // Convert to unit-circle vectors and add
      const aRad = (a / NBINS) * 2 * Math.PI;
      const bRad = (b / NBINS) * 2 * Math.PI;
      const xs = Math.cos(aRad) * aw + Math.cos(bRad) * bw;
      const ys = Math.sin(aRad) * aw + Math.sin(bRad) * bw;
      const newBin = Math.round((Math.atan2(ys, xs) / (2 * Math.PI)) * NBINS) % NBINS;
      chosenI = newBin;
    }
  }
  // Convert bin back to angle. Stitch rows run PERPENDICULAR to the
  // dominant edge so they cross the short dimension.
  const edgeAngle = (chosenI / NBINS) * 2 * Math.PI;
  const stitchAngle = edgeAngle + Math.PI / 2;
  return stitchAngle;
}
...
/* ── Decide stitch strategy from PCA & physical shape ──────────────────── */
function v70_classify(pts, pca, pxPerMm) {
  ...
  /* Tier-3: dispatch on shape. Strategy determines stitching recipe. */
  let type, strategy;
  if (areaMm2 < 1.5)                                { type = "running";  strategy = "running"; }
  else if (widthMm < 0.6)                           { type = "running";  strategy = "running"; }
  else if (widthMm <= 1.4)                          { type = "satin";    strategy = "singleSatin"; }
  else if (widthMm <= 3.5 && pca.aspect > 3.0)      { type = "satin";    strategy = "satin"; }
  else if (widthMm <= 5.5 && pca.aspect < 1.8 && areaMm2 >= 8){
    type = "fill"; strategy = "radialTatami";
  }
  else                                              { type = "fill";     strategy = "tatami"; }
  return { type, strategy, areaMm2, widthMm, aspect: pca.aspect };
}

/* ── Tier-3: radial tatami from centroid for round blob regions ───────── */
function v70_radialStitches(mask, w, h, offX, offY, color, pca, pxScale) {
  const stitches = [];
  const cx = pca.cx, cy = pca.cy;
  let maxR = 0;
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
  ...
  if (maxR < 2) return [];
  const N_ARMS = Math.max(8, Math.min(28, Math.round((2 * Math.PI * maxR) / Math.max(6, 4 * pxScale))));
  const dArm = (2 * Math.PI) / N_ARMS;
  const ROW_SPACING_PX = Math.max(2, 3.5 * pxScale);
  for (let a = 0; a < N_ARMS; a++) {
    const ang = a * dArm;
    const dxA = Math.cos(ang), dyA = Math.sin(ang);
    let r = ROW_SPACING_PX;
    while (r < maxR) {
      const x = Math.round(cx + dxA * r);
      const y = Math.round(cy + dyA * r);
      if (x < 0 || y < 0 || x >= w || y >= h) break;
      if (mask[y * w + x]) {
        stitches.push({ x: offX + x, y: offY + y, color, type: "fill" });
      }
      r += ROW_SPACING_PX;
    }
  }
...
function v70_holdoutBackground(pixMap, N) {
  if (process.env.STICHAI_SKIP_BG === "0") return;
  const total = N * N;
  const seen = new Uint8Array(total);
  const stack = new Int32Array(total);
  const seeds = [0, N - 1, (N - 1) * N, N * N - 1, (N >> 1)];
  let removed = 0;
  for (const s0 of seeds) {
    ...
    while (top > 0) {
      ...
      const x = i % N, y = (i / N) | 0;
      if (x > 0   && !seen[i-1] && pixMap[i-1] === col) { seen[i-1]=1; stack[top++]=i-1; }
      if (x < N-1 && !seen[i+1] && pixMap[i+1] === col) { seen[i+1]=1; stack[top++]=i+1; }
      if (y > 0   && !seen[i-N] && pixMap[i-N] === col) { seen[i-N]=1; stack[top++]=i-N; }
      if (y < N-1 && !seen[i+N] && pixMap[i+N] === col) { seen[i+N]=1; stack[top++]=i+N; }
    }
    if (comp.length > total * 0.05) { for (const i of comp) pixMap[i] = -1; removed += comp.length; }
  }
  if (removed) console.log(`[v70] background held out: ${removed}px (${(100*removed/total).toFixed(1)}%) corner-flood`);
}

function v70_buildShapes(pixMap, colors, canvasSize, pxPerMm, params) {
  const P = params || {};
  const tune = P.tune || {};
  const minAreaPx = Math.max(50, Math.round((tune.v70MinArea || 2.0) * pxPerMm * pxPerMm));
  ...
  /* ── Simple per-component regions (the approach that WORKS) ─────────────
     ...
     kills fill density). */
  v70_holdoutBackground(pixMap, canvasSize);
...
function v70_generateStitches(shapes, colors, params, canvasSize) {
  ...
  /* Tier-5f: skin-fold guard.
   *
   * Adjacent regions that touch pure white background can leak a stitch
   * row out past the silhouette ("bleed row"). We don't want that — the
   * 1-pixel inset forces the boundary inward by a hairline, eliminating
   * the visible spike. Runs cheaply; only affects shapes that touch
   * canvas edge AND are background-facing (warm-light colors).
   */
  for (const sh of shapes) {
    ...
    const touchesEdge = (
      sh.bounds.mnx <= 0 ||
      sh.bounds.mny <= 0 ||
      sh.bounds.mxx >= canvasSize - 1 ||
      sh.bounds.mxy >= canvasSize - 1
    );
    if (!touchesEdge) continue;
    // Erode mask by 1 pixel
    const w = sh.w, h = sh.h;
        ...
        if (sh.mask[y * w + x] === 1) {
          ...
          for (let dy = -1; dy <= 1 && ok; dy++) {
            for (let dx = -1; dx <= 1 && ok; dx++) {
              ...
              if (sh.mask[(y + dy) * w + (x + dx)] !== 1) ok = 0;
            }
          }
          if (ok) newMask[y * w + x] = 1;
        }
      }
    }
    sh.mask = newMask;
    sh.ptCount = Array.from(newMask).filter(v => v).length;
  }

  /* Tier-5b: stitch-budget guard. If projected stitch count exceeds the
   *  budget (default 40 000), progressively downgrade strategies so we
   *  don't ship a 60k-stitch file that the customer machine chokes on.
   *  - radialTatami -> tatami (radial is +40% stitches over plain)
   *  - satin        -> singleSatin
   *  - tatami       -> unchanged
   *  - singleSatin  -> unchanged
   *  We then widen pRow by 1.5x for any region still in tatami.
   *  Returns the new shape list (in place mutation is fine — caller passes the list). */
  if (Array.isArray(shapes)) {
    const PROJ_PER_MM_TATAMI   = 8;     // stitches per mm^2 base, very approximate
    const PROJ_PER_MM_RADIAL   = 11;
    const PROJ_PER_MM_SATIN    = 14;
    const PROJ_PER_MM_SINGLE   = 7;
    const PROJ_PER_MM_RUNNING  = 5;
    let projected = 0;
    for (const sh of shapes) {
      const s = sh.strategy || (sh.type === "fill" ? "tatami" : sh.type === "satin" ? "satin" : "running");
      let per = PROJ_PER_MM_TATAMI;
      if (s === "radialTatami")  per = PROJ_PER_MM_RADIAL;
      else if (s === "satin")   per = PROJ_PER_MM_SATIN;
      else if (s === "singleSatin") per = PROJ_PER_MM_SINGLE;
      else if (s === "running") per = PROJ_PER_MM_RUNNING;
      projected += sh.areaMm2 * per;
    }
    const budget = P.stitchBudget || 40000;
    if (projected > budget) {
      console.log(`[v70-budget] projected ${Math.round(projected)} > budget ${budget}. Downgrading.`);
      for (const sh of shapes) {
        if (sh.strategy === "radialTatami") sh.strategy = "tatami";
        else if (sh.strategy === "satin") sh.strategy = "tatami";  // drop satin to wider-row tatami
      }
    }
    if (projected > budget * 1.4) {
      // severe over-budget: also widen row spacing for everyone
      console.log(`[v70-budget] severe over-budget (${Math.round(projected)} > ${budget*1.4}). pRow widened.`);
      for (const sh of shapes) {
        sh.areaMm2 *= 1.0; // marker, but we'll set wider pRow in dispatch
        sh._wideRow = true;
      }
    }
  }
  ...
  for (let ci = 0; ci < colors.length; ci++) {
    ...
    const _nnChain = (arr, startX, startY) => {
      ...
      while (remaining.length) {
        ...
        for (let k = 0; k < remaining.length; k++) {
          const c = remaining[k]._start || _centroid(remaining[k]);
          const d = (c.x - cx) * (c.x - cx) + (c.y - cy) * (c.y - cy);
          if (d < bd) { bd = d; bi = k; }
        }
        const next = remaining.splice(bi, 1)[0];
        ordered.push(next);
        const c = (next._start || _centroid(next)); cx = c.x; cy = c.y;
      }
      return ordered;
    };
    for (const sh of group) { if (!sh._start) { const _p = v70_traceOutline(sh.mask, sh.w, sh.h); sh._path = _p; sh._start = _p.length ? { x: _p[0][0] + sh.offX, y: _p[0][1] + sh.offY } : { x: sh.offX + sh.w/2, y: sh.offY + sh.h/2 }; } }
    ...
    for (const sh of group) {
      /* Trim if moving far */
      const path = sh._path || v70_traceOutline(sh.mask, sh.w, sh.h);
      ...
      /* Stitch angle — Tier-2: contour-aware when the region is not a clear bar.
         - If clearly elongated (as aspect ≥ 2.2) -> use PCA long axis (existing good).
         - If blob (aspect < 2.2) -> use CONTRAST-aligned angle (dominant boundary
           straight edge). This rotates tatami rows with the contour so shoulders,
           cheeks, garments read as embroidered contours instead of uniform vertical
           stripes. Falls back to perpendicular-to-edge dominant if trace fails. */
      let stitchAngle;
      if (sh.pca.aspect >= 2.2) {
        stitchAngle = sh.pca.angle;
      } else {
        stitchAngle = v70_contourAlignedAngle(
          sh.mask, sh.w, sh.h,
          sh.pca.longAngle  /* safety fallback */
        );
      }

      /* MAIN STITCHING — Tier-3 strategy dispatch
       * running        -> single line of running stitches (eyes / mouth).
       * singleSatin    -> one satin stroke down the long axis (lip / collar).
       * satin          -> tatami-with-tight-row-pitch (sleeve / ribbon).
       * radialTatami   -> radial rows from centroid (round blobs: face cheek).
       * tatami         -> standard parallel rows across short axis (large flats).
       */
      const strat = sh.strategy || (sh.type === "fill" ? "tatami"
        : sh.type === "satin" ? "satin" : "running");
      if (strat === "running") {
        // nothing more to do; outline IS the running stitch.
      } else if (strat === "singleSatin") {
        // Single straight satin stroke down the long axis through PCA centroid.
        // Snip mask range endpoints along long-axis (where mask is non-empty).
        const lc = Math.cos(sh.pca.longAngle), ls = Math.sin(sh.pca.longAngle);
        let uMin = Infinity, uMax = -Infinity;
        for (let y = 0; y < sh.h; y++) for (let x = 0; x < sh.w; x++) {
          if (!sh.mask[y * sh.w + x]) continue;
          const u = (x - sh.pca.cx) * lc + (y - sh.pca.cy) * ls;
          if (u < uMin) uMin = u;
          if (u > uMax) uMax = u;
        }
        const A = { x: sh.offX + sh.pca.cx + lc * uMin, y: sh.offY + sh.pca.cy + ls * uMin };
        const B = { x: sh.offX + sh.pca.cx + lc * uMax, y: sh.offY + sh.pca.cy + ls * uMax };
        const len = Math.hypot(B.x - A.x, B.y - A.y);
        const segs = Math.max(2, Math.ceil(len / Math.max(2, 2.5 * pxScale)));
        for (let k = 0; k <= segs; k++) {
          const t = k / segs;
          const sx = A.x + (B.x - A.x) * t;
          const sy = A.y + (B.y - A.y) * t;
          out.push({ x: sx, y: sy, color, type: "satin" });
          colorCounts[ci].satin++;
          lastX = sx; lastY = sy;
        }
      } else if (strat === "radialTatami") {
        const fs = v70_radialStitches(sh.mask, sh.w, sh.h, sh.offX, sh.offY, color, sh.pca, pxScale);
        for (const s of fs) {
          out.push(s);
          colorCounts[ci].fill++;
          lastX = s.x; lastY = s.y;
        }
      } else if (strat === "tatami") {
        /* Tier-4: per-region thread width (row spacing) by area.
            Tiny regions get denser rows so they don't go transparent,
            large regions get looser rows to keep below the stitch budget. */
        const localPRow = (() => {
          const wideRow = sh._wideRow ? 1.5 : 1;
          const a = sh.areaMm2;
          if (a <  3)  return Math.max(2, Math.round(1.0 * pxScale * wideRow));
          if (a < 15)  return Math.max(2, Math.round(0.7 * pxScale * wideRow));
          if (a < 40)  return Math.max(3, Math.round(0.5 * pxScale * wideRow));
          if (a < 90)  return Math.max(3, Math.round(0.6 * pxScale * wideRow));
          return         Math.max(4, Math.round(0.75 * pxScale * wideRow));
        })();
        const scan = v70_scanRuns(sh.mask, sh.w, sh.h, sh.offX, sh.offY,
                                  stitchAngle, localPRow);
        const fs = v70_runsToStitches(scan, color, pBrick, pPullComp, pMaxBridge, maxStitchLen);
        if (fs.length < 10 && sh.ptCount > 1000) {
          console.log(`[v70-diag] fill ${color} (${sh.ptCount}px, ${sh.w}×${sh.h}) → scanRuns=${scan.length} stitches=${fs.length} angle=${stitchAngle.toFixed(2)} pRow=${localPRow}`);
        }
      ...
      /* running/radial/singleSatin handled above */
    }
  }
  ...
  /* ── Tier-4: connector lines (visually unify near-identical adjacent colors) ──
   *
   * Adjacent regions with color distance dE < 8 (e.g. child's light-blue
   * next to mother's teal, cream next to purple) often produce a visible
   * seam: the engine stitches each region separately, which creates 6+
   * dark contour-spikes where they meet. Connector lines run a thin
   * hairline stitch across the boundary in the dominant colour, so the
   * two regions merge visually. Limits under 600 stitches total.
   */
  if (shapes.length >= 2) {
    const cStep = Math.max(8, Math.round(2.0 * pxScale));
    let cCount = 0;
    const maxC = 600;
    for (let i = 0; i < shapes.length && cCount < maxC; i++) {
      const A = shapes[i];
      if (A.areaMm2 < 4) continue;     // skip tiny
      for (let j = i + 1; j < shapes.length && cCount < maxC; j++) {
        const B = shapes[j];
        if (B.areaMm2 < 4) continue;
        // Same-color connectors would just thicken; only span hue neighbours.
        if (normHex(A.color) === normHex(B.color)) continue;
        const labA = rgbToLab(hexToRgb(A.color));
        const labB = rgbToLab(hexToRgb(B.color));
        if (dE(labA, labB) > 8)  continue;
        // Find shortest distance between their bounding boxes (px).
        const dx = Math.max(0, Math.max(A.bounds.mnx, B.bounds.mnx) - Math.min(A.bounds.mxx, B.bounds.mxx));
        const dy = Math.max(0, Math.max(A.bounds.mny, B.bounds.mny) - Math.min(A.bounds.mxy, B.bounds.mxy));
        const gapPx = Math.hypot(dx, dy);
        if (gapPx > 30 * pxScale) continue;     // skip far apart
        // Emit 6-12 hairline stitches bridging centroid of A → centroid of B
        // along their connection. Use group colour.
        const conn = normHex(A.color);
        const cAx = A.offX + A.pca.cx, cAy = A.offY + A.pca.cy;
        const cBx = B.offX + B.pca.cx, cBy = B.offY + B.pca.cy;
        const segs = 8;
        // TRIM into start of connector
        out.push({ x: lastX, y: lastY, color: conn, type: "trim" });
        for (let s = 0; s <= segs; s++) {
          const t = s / segs;
          out.push({
            x: cAx + (cBx - cAx) * t,
            y: cAy + (cBy - cAy) * t,
            color: conn, type: "running"
          });
          cCount++;
          lastX = cAx + (cBx - cAx) * t; lastY = cAy + (cBy - cAy) * t;
        }
      }
    }
    if (cCount > 0) console.log(`[v70] connector lines: ${cCount} running stitches across ${(cCount/9)|0} bridges`);
  }
...
function v72_buildAndGenerate(pixMap, colors, canvasSize, pxPerMm, params) {
  ...
  const pxScale = canvasSize / 800;
  const tune = P.tune || {};
  const minAreaPx = Math.max(50, Math.round((tune.v72MinArea || 2.0) * pxPerMm * pxPerMm));
  ...
  v70_holdoutBackground(fillSrc, canvasSize);
  ...
  // Fragment drop threshold: tune via params.tune.v72FragPx, but never drop
  // more than the region-finding threshold itself (minAreaPx) would have already.
  const _minFragPx = Math.max(20, Math.round((P.tune && P.tune.v72FragPx || 1.5) * pxPerMm * pxPerMm)); // step5: drop stray fragments — floor lowered so low tune.v72FragPx values take effect
  ...
  for (const ci of ciOrder) {
    ...
    for (const sh of group) {
      ...
      if (sh.ptCount < _minFragPx) continue; // step5: drop all tiny fragments (any type)
    ...
    // Tie-off once at the LAST stitch of this colour (fill or running)
    if (_tiedIn && _lastPt) {
      for (const t of generateTieStitches(_lastPt.x, _lastPt.y, colors[ci], -1, 0)) out.push(t);
    }
  }
...
/* ─── REGION EXTRACTION ──────────────────────────────────*/
/* Despeckle single-pixel palette-flips that show up after quantize:
   pixels of color X with zero 4-connected same-color neighbours get
   re-assigned to the most common neighbour colour. Two-pixel features
   (eyes, pupils, mouth bits) are preserved — only true 1-pixel specks
   vanish. This drops the "138 running regions" pathology we saw on
   fragmented cartoon output. */
function despecklePixMap(pixMap, canvasSize) {
  const out = new Int16Array(pixMap);
  const N = canvasSize;
  for (let y = 1; y < N - 1; y++) {
    for (let x = 1; x < N - 1; x++) {
      const i = y * N + x;
      const c = out[i];
      if (c < 0) continue;
      // count same-colour 4-neighbours
      let same = 0;
      if (out[i - 1]   === c) same++;
      if (out[i + 1]   === c) same++;
      if (out[i - N]   === c) same++;
      if (out[i + N]   === c) same++;
      if (same === 0) {
        // speck — pick majority 4-neighbour colour
        const n4 = [out[i - 1], out[i + 1], out[i - N], out[i + N]];
        const counts = {};
        ...
        if (best >= 0 && bestCount >= 2) out[i] = best;
      }
    }
  }
  return out;
...
function extractRegions(pixMap, colors, canvasSize, mode, opts) {
  const useDespeckle = !opts || opts.despeckle !== false;
  if (useDespeckle) pixMap = despecklePixMap(pixMap, canvasSize);
  ...
  for(let ci=0;ci<colors.length;ci++){
    for(let sy=0;sy<canvasSize;sy++){
      for(let sx=0;sx<canvasSize;sx++){
        ...
        const scaledMinArea = Math.max(8, Math.round(MIN_AREA * 0.6 * Math.pow(canvasSize / 800, 2)));  // 0.6: drops more tiny running regions (was 0.4)
