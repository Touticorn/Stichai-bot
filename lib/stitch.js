...
function generateStitchesFromRegions(pixMap, regions, colors, params, canvasSize) {
  ...
  if (params.definitionOutline !== false) {
    ...
    const oStep = Math.max(6, Math.round(1.8 * _pxmm));     // ~1.8mm running stitch
    const minOutlineArea = Math.max(30, Math.round(2 * _pxmm * _pxmm)); // skip specks
    ...
    for (const reg of regions) {
      ...
      if (bw * bh < minOutlineArea) continue;              // skip tiny specks
      const ci = colors.findIndex(c => normHex(c) === normHex(reg.color));
      if (ci < 0) continue;
      const path = generateOutline(pixMap, reg, ci, canvasSize, outlineHex, oStep);
      if (path.length < 3) continue;

      // Jump to the outline start (trim, never a sewn line across)
      const s = path[0];
      if (oLastX !== -1 && Math.hypot(s.x - oLastX, s.y - oLastY) > 3) {
        stitches.push({x: oLastX, y: oLastY, color: outlineHex, type: "trim"});
        stitches.push({x: s.x, y: s.y, color: outlineHex, type: "trim"});
      }
      // Close the loop back to start for a clean contour
      const closed = path.concat([{x: s.x, y: s.y, color: outlineHex, type: "running"}]);
      for (const p of closed) stitches.push(p);
      outlineStitchCount += closed.length;
      oLastX = s.x; oLastY = s.y;
    }
