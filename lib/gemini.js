...
const GEMINI_MODELS  = [
  "gemini-2.5-flash",
  "gemini-2.5-pro",
  // NOTE: gemini-3.1-flash removed — it returns 404 (not a real model) and wasted a
  // fallback hop on every palette call (see 2026-06-15 logs).
];
const CARTOON_MODELS = ["gemini-2.5-flash-image"]; // dropped 3.1-flash-image-preview (404)
...
- Preserve the person's EXPRESSION and gaze (smiling vs neutral, where they look) so it still reads as them — but render the face with the FEWEST possible marks. A whole face is only: two eyes, two short eyebrows, one short nose mark, one mouth line. Nothing else.
- FORBIDDEN on faces (these make embroidery look scribbled): wrinkles, laugh lines, nasolabial folds, under-eye lines, philtrum, cheek/forehead/jaw shading, double or sketchy lines, stubble, individual teeth, nostril detail. A 40-year-old and a baby get the SAME small number of face marks — age comes from shape, not extra lines.
...
- Eyes, eyebrows, nose, mouth: each a SINGLE clean dark stroke or simple closed shape, well separated from the others (clear flat skin between every feature — features must never touch or merge into a dark mass).
...
- Facial features = at most 6 simple dark marks total per face (2 eyes, 2 brows, 1 nose, 1 mouth), each a single thin stroke on flat skin, separated by clear skin. NO wrinkles, folds, shading, or extra lines of ANY kind.
...
`;
  ...
  const FLASH_FALLBACK_ADDENDUM = `

ADDITIONAL RULES (fallback model -- these are critical):
- Output exactly ONE instance of the subject, centered, filling the frame. NEVER repeat, tile, mirror, or duplicate the subject side-by-side or in a grid -- ONE subject only.
- Skin tone must be a natural tan/beige/brown appropriate to the subject (e.g. #C8A878, #E0B088, #A0704A). NEVER bright or saturated yellow, NEVER #FFBF3F or similar.`;

  const IMAGE_MODELS = [
    { model: "gemini-3-pro-image",     attempts: 2, timeout: 90000, addendum: "" },
    { model: "gemini-3.1-flash-image", attempts: 1, timeout: 50000, addendum: FLASH_FALLBACK_ADDENDUM },
    { model: "gemini-2.5-flash-image", attempts: 1, timeout: 50000, addendum: FLASH_FALLBACK_ADDENDUM },
  ];
  for (const { model, attempts, timeout, addendum } of IMAGE_MODELS) {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${GEMINI_API_KEY}`;
    for (let attempt = 1; attempt <= attempts; attempt++) {
      try {
        const res = await axios.post(url, {
          contents: [{ role: "user", parts: [
            { text: promptText + addendum },
            { inlineData: { mimeType: mime || "image/jpeg", data: b64 } }
          ]}],
        ...
        for (const part of parts) {
          if (part.inlineData?.data) {
            const buf = Buffer.from(part.inlineData.data, "base64");
            console.log(`[extract-cartoon] ${model} attempt ${attempt}/${attempts} -> ${buf.length} bytes, ${nColors} colors`);
            return { buffer: buf, mime: part.inlineData.mimeType || "image/png", model, colorCount: nColors };
          }
        }
        console.warn(`[extract-cartoon] ${model} attempt ${attempt}/${attempts} returned no image`);
      } catch (e) {
        console.warn(`[extract-cartoon] ${model} attempt ${attempt}/${attempts} failed: ${e.response?.status || e.code || ""} ${e.response?.data?.error?.message || e.message}`);
      }
...
- IMPORTANT: keep the subject's EXPRESSION and mood (smiling vs neutral, gaze direction) as in the original — do NOT exaggerate or neutralise it. Likeness comes from face SHAPE and pose, NOT from adding detail lines.
...
- Draw it the way an EMBROIDERY DIGITIZER prepares artwork: a few distinct
  ...
  separating them. Faces use the FEWEST marks: 2 eyes, 2 brows, 1 nose, 1 mouth — single thin strokes, well separated, NO wrinkles/folds/shading/extra lines.
...
- Remove all photographic noise and subtle shading`;
  ...
  const CARTOON_FLASH_ADDENDUM = `

ADDITIONAL RULES (fallback model -- these are critical):
- Output exactly ONE instance of the subject, centered, filling the frame. NEVER repeat, tile, mirror, or duplicate the subject side-by-side or in a grid -- ONE subject only.
- Skin tone must be a natural tan/beige/brown appropriate to the subject (e.g. #C8A878, #E0B088, #A0704A). NEVER bright or saturated yellow, NEVER #FFBF3F or similar.
- Facial features = at most 6 simple dark marks total per face (2 eyes, 2 brows, 1 nose, 1 mouth). NO wrinkles, folds, shading, or extra lines.`;

  const CARTOON_MODEL_LIST = [
    { model: "gemini-3-pro-image",          attempts: 2, timeout: 90000, addendum: "" },
    { model: "gemini-2.5-flash-image",      attempts: 1, timeout: 50000, addendum: CARTOON_FLASH_ADDENDUM },
    { model: "gemini-3.1-flash-image-preview", attempts: 1, timeout: 50000, addendum: CARTOON_FLASH_ADDENDUM },
  ];
  for (const { model, attempts, timeout, addendum } of CARTOON_MODEL_LIST) {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${GEMINI_API_KEY}`;
    for (let attempt = 1; attempt <= attempts; attempt++) {
      try {
        const res = await axios.post(url, {
        ...
        for (const part of parts) {
          if (part.inlineData?.data) {
            const buf = Buffer.from(part.inlineData.data, "base64");
            console.log(`[cartoon] ${model} attempt ${attempt}/${attempts} -> ${buf.length} bytes`);
            return { buffer: buf, mime: part.inlineData.mimeType || "image/png" };
          }
        }
        console.warn(`[cartoon] ${model} attempt ${attempt}/${attempts} returned no image`);
      } catch (e) {
        console.warn(`[cartoon] ${model} attempt ${attempt}/${attempts} failed: ${e.response?.status || e.code || ""} ${e.response?.data?.error?.message || e.message}`);
      }
      if (attempt < attempts) await new Promise(r => setTimeout(r, 1000));
    }
  }
