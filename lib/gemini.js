...
- Extract just the subject; replace everything else with a plain white background
- Use EXACTLY ${nColors} solid flat colours — no gradients, no shadows, no photographic texture
- CRITICAL FOR EMBROIDERY: simplify small repeating patterns (polka dots, tiny prints, strawberries, florals, stripes) into ONE solid colour block. Do NOT reproduce tiny scattered details — they cannot be sewn.
...
- Style: bold vector sticker / appliqué patch — simple enough to embroider with thread
...
- CRITICAL: simplify small repeating patterns (polka dots, tiny prints, florals, stripes) into ONE solid colour block. Do NOT reproduce tiny scattered details — they cannot be sewn on an embroidery machine.
...
- Style: bold vector sticker / appliqué patch
