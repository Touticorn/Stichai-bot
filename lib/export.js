...
function encodeDST(stitches, machineLimits) {
  ...
  // Center the design at the origin (standard DST practice): translate every
  // stitch by -center so the first jump is short and the design sits centered
  // in the hoop instead of being offset from a corner.
  if (filtered.length) {
    let cmnx = Infinity, cmxx = -Infinity, cmny = Infinity, cmxy = -Infinity;
    for (const s of filtered) {
      if (s.x < cmnx) cmnx = s.x; if (s.x > cmxx) cmxx = s.x;
      if (s.y < cmny) cmny = s.y; if (s.y > cmxy) cmxy = s.y;
    }
    const cx = (cmnx + cmxx) / 2, cy = (cmny + cmxy) / 2;
    filtered = filtered.map(s => ({ ...s, x: s.x - cx, y: s.y - cy }));
  }
  ...
  let stitchCount = 0;
  let colorChanges = 0;
  const usedColors = [];   // hex colours that actually produced sewn stitches, in order
  ...
  let needJump = true;   // the initial move from origin to the first stitch must be a jump
  ...
  for (const s of filtered) {
    ...
    if (s.color !== lastColor) {
      usedColors.push(s.color);     // first appearance of this colour block
    }
    lastColor = s.color;
  ...
  const buffer = Buffer.concat([header, ...recs]);
  // Real, accurate counts that match what was actually written to the DST.
  buffer.meta = {
    stitchCount,                       // includes expanded jumps
    colorBlocks: colorChanges + 1,     // actual colour blocks in the file
    usedColors,                        // ordered list of hex colours actually sewn
  };
  return buffer;
}
